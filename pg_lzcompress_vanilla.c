...
#define SEG_HEAD_SIZE 6;

/* ----------
 * pglz_segment_compress -
 *
 *		Compresses source into dest using strategy. Returns the number of
 *		bytes written in buffer dest, or -1 if compression fails.
 * ----------
...
pglz_compress_hacked_seg(const char *source, int32 slen, char *dest,
					  const PGLZ_Strategy *strategy)
{
	unsigned char *bp = (unsigned char *) dest + SEG_HEAD_SIZE;
	unsigned char *bstart = (unsigned char*)dest;
	int			hist_next = 1;
	bool		hist_recycle = false;
	const char *dp = source;
	const char *dend = source + slen;
	unsigned char ctrl_dummy = 0;
	unsigned char *ctrlp = &ctrl_dummy;
	unsigned char *segp = (unsigned char *) bstart;
	unsigned char ctrlb = 0;
	unsigned char ctrl = 0;
	bool		found_match = false;
	int32		match_len;
	...
	int32		need_rate;
	/*
	 * We can set the max segment length in the pglz strategy.
	 */
	int16       max_seg_len = TOAST_MAX_CHUNK_SIZE;
	int			hashsz;
	int			mask;
	...
	while (dp < dend)
	{
		...
		/*
		 * If the current segment length is greater than the maximum segment length,
		 * set the current segment header and initialize the new segment.
		 */

		if (bp - segp + 4 > max_seg_len)
		{
			*((int32*)segp) = (int32)(dp - source);
			*((int16*)(segp + sizeof(int32))) = (int16)(bp - segp);
			memset(hist_start, 0, hashsz * sizeof(int16));
			segp = bp;
			bp += SEG_HEAD_SIZE;
			*ctrlp = ctrlb;
